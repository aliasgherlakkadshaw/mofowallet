...
module.controller('WalletController', function($scope, $rootScope, WalletService, plugins, $translate) {
  ...
  $scope.remove = function (a) {
    plugins.get('alerts').confirm({
      title: $translate.instant('translate.remove_key'),
      message: $translate.instant('translate.remove_key_message')
    }).then(
      function (confirmed) {
        if (confirmed) {
