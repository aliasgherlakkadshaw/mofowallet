...
module.controller('secretPhraseModalController', function (items, $modalInstance, $scope, $translate, 
  $timeout, nxt, modals, plugins, $sce, db, KeyService) {

  var walletPlugin              = plugins.get('wallet');

  $scope.SIMPLE                 = $translate.instant('translate.simple');
  $scope.COMPLEX                = $translate.instant('translate.complex');
  ...
  if ($scope.state == $scope.SIMPLE) {
    $scope.items.messageHTML    = $sce.getTrustedHtml(items.messageHTML|| $translate.instant('translate.simple_message'));    
  }
  else {
    $scope.items.messageHTML    = $sce.getTrustedHtml(items.messageHTML|| $translate.instant('translate.complex_message'));
